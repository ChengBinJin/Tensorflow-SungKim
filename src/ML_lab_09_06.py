...
class Model:
    ...
    -----------
    X: tf.float32
        This is a tesorflow placeholder for MNIST images
    ...
    def __init__(self, name, input_dim, output_dim, hidden_dims=[32, 32], use_batchnorm=True, activation_fn=tf.nn.relu,
        ...
        use_batchnorm: bool (default: True)
            If true, it will create teh batchnormalization layer
...
# We create two models: one with the batch norm and other wihout
...
# Usually we only need on solver class
...
plot_compare(train_accs, ylim=[0., 1.], title='Training Acc at Epoch')
plot_compare(valid_losses, title='Validation Loss at Epoch')
plot_compare(valid_accs, ylim=[0., 1.], title='Validation Acc at Epoch')
